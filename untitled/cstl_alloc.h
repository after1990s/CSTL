...
template <class ForwardItr, class Size, Size T>
...
template <class ForwardItr, class Size, Size T>
inline ForwardItr uninitialized_fill_n(ForwardItr first, ForwardItr last, const T & x) {
    return __uninitialized_fill_n(first, last, x, value_type(first));
}

template <class ForwardItr, class Size, class T, class T1>
inline ForwardItr __uninitialized_fill_n(ForwardItr first, ForwardItr last, const T& x, T1*) {
    typedef typename __type_traits<T1>::is_POD_type is_POD;
    return __uninitialized_fill_n_aux(first, last, x, is_POD());
}

template <class ForwardItr, class Size, Size T>
ForwardItr __uninitialized_fill_n_aux(ForwardItr first, ForwardItr last, const T& x, __true_type) {
    return fill(first, last, x);
}

template <class ForwardItr, class Size, Size T>
ForwardItr __uninitialized_fill_n_aux(ForwardItr first, ForwardItr last, const T& x, __false_type) {
    ForwardItr cur = first;
    for (; cur != last; ++cur) {
        _construct(&*cur, x);
    }
