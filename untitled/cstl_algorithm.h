...
inline void copy(InputItr first, InputItr last, OutputItr res) {
    return __copy_dispatch<InputItr, OutputItr>()(first, last, res);
}

template<>
inline void copy(const char *first, const char * last, const char *res) {
    MoveMemory((void*)res, (void*)first, last - first);
}


template<>
inline void copy(const wchar_t *first, const wchar_t * last, const wchar_t *res) {
